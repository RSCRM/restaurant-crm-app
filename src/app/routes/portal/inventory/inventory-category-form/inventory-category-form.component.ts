...
import { CreateInventoryCategoryRequest, InventoryCategoryResponse, UpdateInventoryCategoryRequest } from '../inventory.model';
...
export class InventoryCategoryFormComponent implements OnInit {
  ...
  private readonly modalData = inject<InventoryCategoryResponse | null>(NZ_MODAL_DATA, { optional: true });

  loading = false;
  isEdit = false;

  form = this.fb.group({
    categoryName: this.fb.control('', [Validators.required, Validators.maxLength(255)]),
  ...
  submit(): void {
    ...
    if (this.isEdit && this.modalData) {
      ...
      this.inventoryService
        ...
        .pipe(
          ...
          catchError(() => {
            this.message.error('Update inventory category failed');
            return EMPTY;
          }),
          finalize(() => {
            this.loading = false;
    ...
    this.inventoryService
      ...
      .pipe(
        takeUntilDestroyed(this.destroyRef),
        catchError(() => {
          this.message.error('Create inventory category failed');
          return EMPTY;
        }),
        finalize(() => {
          this.loading = false;
