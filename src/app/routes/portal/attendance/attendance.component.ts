...
export class AttendanceComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    // Check permissions
    this.store.select(selectHasPermission('ATTENDANCE_QR_DISPLAY'))
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe(has => {
    ...
    this.store.select(selectHasPermission('ATTENDANCE_SELF_WRITE'))
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe(has => {
    ...
    this.store.select(selectHasPermission('ATTENDANCE_SELF_READ'))
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe(has => {
    ...
    this.store.select(selectHasPermission('ATTENDANCE_BRANCH_READ'))
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe(has => {
  ...
  loadBranchAttendance(): void {
    ...
    this.attendanceService.getBranchAttendance(
      this.formatDate(this.branchDate), this.selectedBranchId ?? undefined)
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe({
        next: data => {
  ...
  loadBranches(organizationId: string): void {
    this.attendanceService.getOrganizationBranches(organizationId)
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe(branches => {
  ...
  loadEmployeeHistory(): void {
    ...
    const from = this.employeeDateRange[0]
      ? this.formatDate(this.employeeDateRange[0]) : null;
    const to = this.employeeDateRange[1]
      ? this.formatDate(this.employeeDateRange[1]) : null;
    this.attendanceService.getEmployeeHistory(
      this.selectedEmployeeId, from, to,
      this.employeeHistoryPage, this.employeeHistorySize,
  ...
  loadCurrentStatus(): void {
    ...
    this.attendanceService.getMyHistory(null, null, 1, 1)
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe({
        next: res => {
  ...
  checkIn(): void {
    ...
    this.attendanceService.checkIn({ qrToken: this.qrTokenInput.trim() })
      .subscribe({
        next: res => {
        ...
        error: err => {
          this.submittingCheck = false;
          this.message.error(getHttpErrorMessage(this.i18n, err, 'attendance.checkin-failed'));
          this.cdr.markForCheck();
        }
      });
  }
  ...
  checkOut(): void {
    ...
    this.attendanceService.checkOut()
      .subscribe({
        ...
        error: err => {
          this.submittingCheck = false;
          this.message.error(getHttpErrorMessage(this.i18n, err, 'attendance.checkout-failed'));
          this.cdr.markForCheck();
        }
      });
  }
  ...
  loadHistory(): void {
    ...
    this.attendanceService.getMyHistory(fromStr, toStr, this.currentPage, this.pageSize)
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe({
        next: res => {
  ...
  private startAttendanceRealtime(): void {
    ...
    this.attendanceEventsSubscription = this.attendanceService
      .watchBranchAttendance(this.selectedBranchId)
      .subscribe(() => {
        this.loadBranchAttendance();
