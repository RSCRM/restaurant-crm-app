...
export class AttendanceService {
  ...
  private readonly api = '/api/v1/erp/attendances';
  ...
  getCurrentQr(branchId?: string, force = false): Observable<AttendanceQrResponse> {
    ...
    return this.http
      .get<ApiResponse<AttendanceQrResponse>>(`${this.api}/qr`, { params: branchId ? { branchId } : {} })
      .pipe(
  ...
  getMyHistory(from: string | null, to: string | null, page = 1, size = 10): Observable<PagingResponse<AttendanceResponse>> {
    ...
    if (to) params['to'] = to;
    return this.http.get<ApiResponse<PagingResponse<AttendanceResponse>>>(`${this.api}/me`, { params }).pipe(map(response => response.data));
  }
  ...
  getOrganizationBranches(organizationId: string): Observable<AttendanceBranchResponse[]> {
    return this.http
      .get<ApiResponse<PagingResponse<AttendanceBranchResponse>>>(
        `/api/v1/erp/organization-branches/organization/${organizationId}`,
  ...
  getEmployeeHistory(
    ...
    return this.http
      .get<ApiResponse<PagingResponse<AttendanceResponse>>>(
        `${this.api}/branch/employees/${employeeId}/history`, { params })
      .pipe(map(response => response.data));
  }
      ...
      void (async () => {
        try {
          const response = await fetch(
            `${this.api}/subscribe?branchId=${encodeURIComponent(branchId)}`,
            {
