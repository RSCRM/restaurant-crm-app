...
export class BranchService {
  ...
  assignBranchManager(branchId: string, request: AssignBranchManagerRequest): Observable<BranchManagerResponse> {
    return this.http
      .put<ApiResponse<BranchManagerResponse>>(`${this.BRANCH_MANAGER_API}/${branchId}/manager`, request)
      .pipe(map(res => res.data));
  }

  removeBranchManager(branchId: string): Observable<null> {
    return this.http.delete<ApiResponse<null>>(`${this.BRANCH_MANAGER_API}/${branchId}/manager`).pipe(map(res => res.data));
  }
}
